...
def lambda_handler(event, context):
    try:
        ...
        yesterday = date.today() - timedelta(days=201)
        ...
        ensure_s3_directory('alpb-ftp-test', ftp_directory, s3_client)
        for filename in filenames:
            s3_key = create_s3_key(ftp_directory, filename)
            ftp_to_s3(ftp, ftp_directory, filename, s3_client, 'alpb-ftp-test', s3_key)
